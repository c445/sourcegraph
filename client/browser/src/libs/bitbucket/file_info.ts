...
import {
    ...
    getMostFileInfoFromMultiFileDiffCodeView,
    getPRIDFromPathName,
} from './scrape'
...
export const resolvePullRequestFileInfo = (codeView: HTMLElement): Observable<FileInfo> => {
    const fileInfo = getMostFileInfoFromMultiFileDiffCodeView(codeView)
...
export const resolveCompareFileInfo = (codeView: HTMLElement): Observable<FileInfo> =>
    of(codeView).pipe(
        map(codeView => {
            ...
            return {
                ...getMostFileInfoFromMultiFileDiffCodeView(codeView),
